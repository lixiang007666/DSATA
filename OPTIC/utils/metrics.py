...
def calculate_metrics(test, reference, model='segformer'):
    if model == 'segformer':
        test = np.array(test)
        if test.ndim == 2:
        ...
        for i in range(batch_size):
            single_test = test[i]
            class_0 = (single_test == 0).astype(np.uint8)
            class_1 = ((single_test == 0) | (single_test == 1)).astype(np.uint8)
            result = np.stack([class_1, class_0], axis=0)
            batch_results.append(result)
        
        test = np.stack(batch_results, axis=0)
        _, reference = data_process(pred=reference, label=reference, threshold=0.5)
    else:
